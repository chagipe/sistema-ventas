      ...
      <div className="ml-60 flex-1 flex flex-col">
        ...
        <main className="mt-16 p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-slate-800 font-semibold text-lg">Control de inventario</h2>
              <p className="text-slate-400 text-sm mt-0.5">{productos.length} productos registrados</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => exportarInventarioExcel(productosFiltrados)}
              ...
              <button
                onClick={() => exportarInventarioPDF(productosFiltrados)}
          ...
          <div className="grid grid-cols-3 gap-5 mb-6">
            <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Total productos</p>
                <p className="text-2xl font-bold text-slate-800 mt-0.5">{productos.length}</p>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-800 mt-0.5">{stockBajo.length}</p>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">Sin stock</p>
                <p className="text-2xl font-bold text-slate-800 mt-0.5">{sinStock.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 mb-4">
            <input
              ...
              className="bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-700 outline-none w-64 placeholder-slate-400"
            />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="text-left px-5 py-3.5 text-slate-500 font-medium">Stock actual</th>
                  <th className="text-left px-5 py-3.5 text-slate-500 font-medium">Stock mín.</th>
                  <th className="text-left px-5 py-3.5 text-slate-500 font-medium">Nivel</th>
                  ...
                  productosFiltrados.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td className="px-5 py-4 text-slate-500">{p.stock_minimo}</td>
                        <td className="px-5 py-4 w-36">
                          <div className="w-full bg-slate-100 rounded-full h-2">
                            <div
                              className={`${barra.color} h-2 rounded-full transition-all`}
